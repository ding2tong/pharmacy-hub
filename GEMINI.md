    ...
    <script type="text/babel">
        ...
        const CalloutComponent = ({ severity, title, content }) => {
            ...
            return (
                <div className={`p-8 rounded-[3rem] border-2 mt-8 shadow-lg ${themes[severity] || themes.info} backdrop-blur-sm`}>
                    ...
                    <p className="text-[1.1rem] font-medium leading-relaxed opacity-80 whitespace-pre-wrap">{content}</p>
                </div>
            );
        };
        ...
        const AccordionComponent = ({ items }) => (
            <div className="mt-8 space-y-4">
                {items.map((item, idx) => (
                    <details key={idx} open className="group border border-white/50 rounded-[2.5rem] overflow-hidden bg-white/30 shadow-sm transition-all duration-300 hover:shadow-md hover:bg-white/50">
            ...
            if (slide.type === 'pdf') {
                return (
                    <div className="slide-content text-center flex flex-col items-center justify-center py-16">
                        ...
                        <p className="text-xl text-gray-500 mb-10 font-medium">點擊下方按鈕以在新視窗開啟詳細文件</p>
                        <a 
                            href={slide.url} 
                            target="_blank" 
                            className="inline-flex items-center gap-4 px-12 py-5 bg-emerald-600 hover:bg-emerald-700 text-white rounded-full font-bold text-xl shadow-xl transition-all hover:scale-105 active:scale-95 no-underline"
                        >
                        ...
                        <div className="mt-12 p-6 bg-amber-50 rounded-3xl border border-amber-100 max-w-lg">
                            <p className="text-sm text-amber-800 leading-relaxed font-medium">
                                💡 <strong>提示：</strong> 開啟後您可以使用瀏覽器的列印或下載功能儲存文件。
                            </p>
                        </div>
                    </div>
                );
        ...
        const App = () => {
            ...
            useEffect(() => {
                ...
                slidesData.forEach(slide => {
                    if (slide.type === 'quiz' && slide.questions && slide.questions.length > 1) {
                        slide.questions.forEach((q, i) => {
                            finalSlides.push({
                                ...slide,
                                id: `${slide.id}-q${i}`,
                                title: `${slide.title} (${i + 1}/${slide.questions.length})`,
                                questions: [q]
                            });
                        });
                    } else {
